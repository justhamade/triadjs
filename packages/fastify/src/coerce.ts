 ...
 *
 * Body coercion is not needed — Fastify parses JSON bodies before the
 * route handler runs, so values already have the right JS types.
 *
 * Design note: we look at the schema's `kind` discriminator rather than
 * `instanceof NumberSchema` etc. so this works with routers loaded via
